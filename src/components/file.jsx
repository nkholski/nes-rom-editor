...
class File extends Component {
  render() {
    let page = null;
    if (
      this.props.romVersion > 0 &&
      Object.keys(this.props.romInfo).length > 0
    ) {
      const prgBytes = this.props.romInfo
        ? 16 * this.props.romInfo.prg.length
        : -1;
      const chrBytes = 8 * this.props.romInfo.chr.length;
      const chrText =
        chrBytes > 0 ? (
          <span>
            {this.props.romInfo.chr.length} bank(s)({chrBytes} kb)
          </span>
        ) : (
          "No CHR-Rom see note below"
        );
      const ramNote =
        chrBytes === 0 ? (
          <span>
            Note: This rom has no CHR-Rom which means it uses CHR-Ram. You will
            need to dig through PRG-Rom to manipulate GFX.
          </span>
        ) : (
          ""
        );
      page = (
        <Container className="nes-container with-title is-centered">
          ...
          <Row>
            <Col>
              name: {this.props.romInfo.name}
              <br />
              filename: {this.props.romInfo.filename}
              <br />
              md5: {this.props.romInfo.md5}
              <br />
              no-intro md5: {this.props.romInfo.noIntroMD5}
              <br />
              prg-rom: {this.props.romInfo.prg.length} bank(s) ({prgBytes} kb)
              <br />
              chr-rom: {chrText}
              <br />
              Hacks: 15
    ...
    return (
      <div id="rom-page" className="row">
        <Container className="nes-container with-title is-centered">
          <p className="title">Welcome to NES Tamp</p>
          <Row>
            <Col>
              NES Tamp the result of a birthday suprise rom hack project going
              overboard and morphed into a tool. It's a general tool for hacking
              nes roms with focus on graphics editing and basic hacks. It has
              support for game specific hacks but not advanced features like
              level editors.
              <h4>Features</h4>
              <ul>
                ...
                <li>
                  Identify rom specific palette settings and alter palettes
                </li>
        ...
        <input
          type="file"
          id="rom-input"
          onChange={e => this.loadRom(e.target.files)}
  ...
  downloadIPS() {
    ...
    for (let i = 0; i < len; i++) {
      const diff =
        this.props.untouchedRom.getUint8(i) !== this.props.romData.getUint8(i);
      ...
      if (noDiffCount === 5 && currentRecord.addr !== -1) {
        currentRecord.values = currentRecord.values.slice(
          0,
          currentRecord.values.length - 5
        );
    ...
    records.forEach(record => {
      ...
      const startAddrString = (
        "000000000000000000000000" + record.addr.toString(2)
      ).slice(-24);
      for (let byte = 0; byte < 3; byte++) {
        const num = parseInt(
          startAddrString.substring(8 * byte, 8 * byte + 8),
          2
        );
      ...
      const sizeString = (
        "0000000000000000" + record.values.length.toString(2)
      ).slice(-16);
  ...
  saveToLocalStorage() {
    const base64RomData = btoa(
      String.fromCharCode(...new Uint8Array(this.props.romData.buffer))
    );
    localStorage.setItem("romData", base64RomData);
    alert("Saved changes to localStorage");
  }
}
...
const mapDispatchToProps = dispatch =>
  bindActionCreators(
    { setRomInfo, storeRom, setRomSettings, loadFile },
    dispatch
  );
