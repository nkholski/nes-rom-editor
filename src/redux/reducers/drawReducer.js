...
export default (state = defaultState, action) => {
  ...
  switch (type) {
    case "SET_PALETTE":
      ...
      const currentColors = [
        nesPalette[state.nesColorIndicies[0]],
        nesPalette[state.nesColorIndicies[1]],
        nesPalette[state.nesColorIndicies[2]],
        nesPalette[state.nesColorIndicies[3]]
      ];
      console.log("PALETE");
      return { ...state, colors: currentColors, nesPalette };
    case "SET_ACTIVE_COLOR":
      const activeColorIndex = payload;
      return { ...state, mode: "draw", activeColorIndex };
    case "SET_COLORS":
