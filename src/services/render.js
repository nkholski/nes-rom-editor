const renderTile = (firstByteIndex, dataView, X, Y, ctx, scale = 1, color, flipX = false, flipY = false) => {
    return (dispatch, getState) => {
        render(firstByteIndex, dataView, X, Y, ctx, scale, color, flipX, flipY);
        console.log("GET STATE", getState());
    }
}

const renderTileBlock = (X, Y, tiles, color, ctx, scale) => {
    return (dispatch, getState) => {
        tiles.forEach((tile) => {
...
const renderTileList = (X, Y, tiles, width, color, ctx, scale) => {
    return (dispatch, getState) => {
        tiles.forEach((tile) => {
...
const render = (firstByteIndex, dataView, X, Y, ctx, scale = 1, color, flipX = false, flipY = false) => {
    /*
